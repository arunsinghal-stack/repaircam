...
class StubCapture(ActiveCapture):
    ...
    def __init__(self, dest: Path, payload: bytes, *, fail: bool = False):
        self.dest = dest
...
class StubBackend(CaptureBackend):
    ...
    def __init__(self, camera: CameraConfig, *, fail: bool = False):
    ...
    def start(self, dest: Path, *, duration: float | None = None) -> StubCapture:
        ...
        capture = StubCapture(dest, f"[{dest.name}]".encode(), fail=bool(self.fail))
